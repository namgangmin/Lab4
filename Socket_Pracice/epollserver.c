...
#define QLEN 6 /* size of request 
		  queue */
#define EPOLL_SIZE 128
#define BUF_SIZE 128
#define MAX_CLNT 4
void * handle_clnt(int epfd, int sockfd);
...
int main(int argc, char *argv[])
{
	...
	ep_events = (struct epoll_event *) 
		malloc(sizeof(struct epoll_event)*EPOLL_SIZE);
	event.events = EPOLLIN;
	event.data.fd = server_sock;
	epoll_ctl(epfd, EPOLL_CTL_ADD, server_sock, 
			&event);
...
void * handle_clnt(int epoll_fd, int client_sock)
{
	...
	client_count--;
	epoll_ctl(epoll_fd, EPOLL_CTL_DEL, client_sock, 
			NULL);
	return NULL;
}
void send_msg(char *msg, int len){
